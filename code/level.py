...
from particle import ParticlesEffect
...
class Level:
    def __init__(self,current_level,surface,create_overworld,change_coins,change_health):
        ...
        # user interface
        self.change_coins = change_coins #24
        ...
        # #bg plams setup
        # tree_bg_layout = import_csv_layout(level_data['bg palms'])
        # self.tree_bg_sprites = self.create_tile_group(tree_bg_layout,'bg palms')
        ...
        #potion
        # potion_layout = import_csv_layout(level_data['potions'])
        # self.potion_sprites = self.create_tile_group(potion_layout,'potions')
        ...
        for row_index,row in enumerate(layout):
            for col_index ,val in enumerate(row):
                if val != '-1':
                    ...
                    # if type == 'bg palms':
                    #     grass_tile_list = import_cut_graphics('./graphics/terrain/trees.png')
                    #     tile_surface = grass_tile_list[int(val)]
                    #     sprite = StaticTile(tile_size,x,y,tile_surface)
    ...
    def check_coin_collisions(self): #25
    ...
    def cheack_health_collision(self):
        collided_health = pygame.sprite.spritecollide(self.player.sprite,itemGroup,True)
        if collided_health:
            for health in collided_health:
                pass
        ...
        if enemy_collisions:
            for enemy in enemy_collisions:
                ...
                if enemy_top < player_bottom < enemy_center and self.player.sprite.direction.y >= 0:    
                    ...
                    explosion_sprite = ParticlesEffect(enemy.rect.center,'explosion')
                    self.explosion_sprites.add(explosion_sprite)
                    self.change_coins(50)
                    random_chance = randint(1,20)
                    print(random_chance)
                    if random_chance >= 1 and random_chance <= 10:
                        item = Item(enemy.rect.x, enemy.rect.y, 0, "./graphics/item/item_health.png")
                        itemGroup.add(item)
                        tmp_currentlevel = self.current_level
                    elif random_chance >= 11 and random_chance <= 20:
                        item = Item(enemy.rect.x, enemy.rect.y, 1, "./graphics/item/item_health.png")
                        itemGroup.add(item)
                        tmp_currentlevel = self.current_level
                    enemy.kill()
    ...
    def run(self):
        ...
        # #tree bg
        # self.tree_bg_sprites.update(self.world_shift)
        # self.tree_bg_sprites.draw(self.display_surface)
        ...
        itemGroup.update(self.world_shift)
