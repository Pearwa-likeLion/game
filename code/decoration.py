...
class Sky:
    def __init__(self,horizon,style = 'level'):
        ...
        if self.style == 'overworld':
            plam_surfaces = import_folder('./graphics/overworld/palms')
            self.plams = []
            
            for surface in [choice(plam_surfaces) for image in range(8)]:
                x = randint(0.,screen_width)
                y = (self.horizon * tile_size) + randint(50,100)
                rect = surface.get_rect(midbottom = (x,y))
                self.plams.append((surface,rect))
                
            cloud_surfaces = import_folder('./graphics/overworld/clouds')
            self.clouds = []
            
            for surface in [choice(cloud_surfaces) for image in range(3)]:
                x = randint(0.,screen_width)
    ...
    def draw(self,surface):
        ...
        if self.style == 'overworld':
            for plam in self.plams:
                surface.blit(plam[0],plam[1])
